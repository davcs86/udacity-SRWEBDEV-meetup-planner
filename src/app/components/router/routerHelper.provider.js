(function() {
   ...
   */
  angular
    .module('davcs86.meetupPlanner.components.router')
  ...
  function routerHelperProvider($locationProvider, $stateProvider, $urlRouterProvider) {
    ...
    function routerHelper(
      ...
      function _handleRoutingErrors() {
        $rootScope.$on('$destroy', $rootScope.$on('$stateChangeError', onEvent));
      ...
      function _updateDocumentTitle() {
        $rootScope.$on('$destroy', $rootScope.$on('$stateChangeSuccess', onEvent));
         ...
         */
        function onEvent(event, toState) {
          stateCounts.changes++;
